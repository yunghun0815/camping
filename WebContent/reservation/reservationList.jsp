...
<h1 class="reservationTitle">예약 페이지</h1>
...
<script type="text/tmpl" id="tmpl">
  ...
  var date = date || new Date(),
      ...
      i, j;
	var dateListEL = '${dateList}'.replace("[","").replace("]","").replace(/(\s*)/g, "");
	var dateList = dateListEL.split(",");
        ...
        <td class="calendar-day {{: dayclass }} {{: thedate.toDateCssClass() }} {{: date.toDateCssClass() === thedate.toDateCssClass() ? 'selected':'' }} {{: daycss[i] }} js-cal-option" data-date="{{: thedate.toISOString() }}">
	      ...
	      <input class="campingNo" name="campingNo" type="hidden" value="1">   
			...
			{{ if (thedate > today) { var checkDate = false;}}
				{{  }}
				{{ for(q = 0; q < num; q++) { }}
					{{ if (dateFormat(thedate) == dateList[q]) { }}
						<span class="reservationNoButton">예약 불가능</span>
						<input class="checkDate" type="hidden" value="false">
					{{checkDate=true; } }}
				{{ } }}

				{{ if (checkDate==false) { }}
					<span class="reservationButton">예약 가능</span>
					<input class="checkDate" type="hidden" value="true">
				{{ } }}
				{{ } }}
...
(function ($) {
  ...
  function calendar($el, options) {
    ...
    $el.on('click', '.js-cal-prev', function () {
      ...
      draw();
    }).on('click', '.js-cal-option', function () {/* 예약 클릭 이벤트 */
      if($(this).find('.reservationCheck').val() == 'true'){
    	  var reservationDate = $(this).find('.reservationDate').val(); 
    	  var campingNo = $(this).find('.campingNo').val();
